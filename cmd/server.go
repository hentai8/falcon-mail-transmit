...
func main() {
	...
	utils.HandlePanic(lmstfy.ConsumeNewProblemMailEvent, lmstfy.ConsumeNewOKMailEvent, lmstfy.ConsumeNewCallbackMailEvent, lmstfy.ConsumeNewProblemFeishuEvent,
		lmstfy.ConsumeNewOKFeishuEvent, lmstfy.ConsumeNewCallbackFeishuEvent)
...
func feishu(c echo.Context) error {
	...
	for {
		if strings.Contains(sub, "故障") {
			redisKey = "problem-feishu-" + GenerateNoWithoutPrefix()
		} else if strings.Contains(sub, "恢复") {
			redisKey = "ok-feishu-" + GenerateNoWithoutPrefix()
		} else if strings.Contains(sub, "回调函数") {
			redisKey = "callback-feishu-" + GenerateNoWithoutPrefix()
		} else {
			// 做规避，防止出现不可预知的情况
			redisKey = "ok-feishu-" + GenerateNoWithoutPrefix()
		}
